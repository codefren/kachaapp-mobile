...
import { ActivityIndicator, Animated, FlatList, Modal, Pressable, RefreshControl, ScrollView, StatusBar, StyleSheet, Text, TextInput, View } from 'react-native';
...
export default function ProvidersScreen() {
  ...
  const calculateOrderProximity = useCallback((orderDates: string[] | undefined) => {
  ...
  const sortProvidersByProximity = useCallback((providers: ProviderItem[]) => {
  ...
  const getTimeContext = useCallback(() => {
    ...
    return {
      ...
      nextOrderDay,
      daysUntilNextOrder: minDaysUntilOrder === Infinity ? null : minDaysUntilOrder
    };
  ...
  const fetchProviders = useCallback(async () => {
    ...
    console.log('[Providers] Token preview:', currentToken?.substring(0, 20) + '...');
    ...
    try {
      ...
      const response = await apiMiddleware.get<ProviderItem[]>('/api/providers/', true);
      
      console.log('[Providers] Response completa desde apiMiddleware:', JSON.stringify(response, null, 2));
  ...
  const fetchHistoricalOrder = useCallback(async () => {
    ...
    try {
      ...
      console.log('[History] 📥 Response completa:', JSON.stringify(response, null, 2));
  ...
  const getOrderInfo = (item: ProviderItem): { inProgress: boolean; status: 'PLACED' | 'DRAFT' | null; poId: number | null } => {
    if (item.has_received_orders && item.has_received_orders.order_id) {
      const status = (item.has_received_orders.status || '').toUpperCase() as 'PLACED' | 'DRAFT';
      if (status === 'PLACED' || status === 'DRAFT') {
        return { inProgress: true, status, poId: item.has_received_orders.order_id };
      }
    }
    if (item.has_draft_reception && item.draft_reception_order_id) {
      return { inProgress: true, status: 'DRAFT', poId: item.draft_reception_order_id };
    }
    return { inProgress: false, status: null, poId: null };
  };

  // Función para calcular tiempo restante de forma simple
  const getTimeRemaining = useCallback((orderDates: string[] | undefined) => {
    if (!orderDates || orderDates.length === 0) {
  ...
  const renderItem = ({ item }: { item: ProviderItem }) => {
    ...
    const leftColor = info.inProgress ? 
      ...
      timeRemaining.urgent ? timeRemaining.color : '#e5e7eb';
      ...
      <Pressable
        ...
        onPress={() => {
          ...
          if (orderId && (status === 'DRAFT' || status === 'PLACED')) {
            ...
            console.log('[Navigation] Navegando a Purchase Order Detail:', orderId);
            router.push({
            ...
            console.log('[Navigation] Navegando a productos del proveedor:', item.id);
            router.push(`/products?provider=${item.id}&provider_name=${encodeURIComponent(item.name)}` as any);
          }
        }}
      >
        <View style={styles.cardContent}>
          <View style={[styles.iconCircle, { backgroundColor: `${leftColor}15`, borderColor: leftColor }]}>
            <Text style={[styles.icon, { color: leftColor }]}>
              {info.status === 'PLACED' ? '■' : info.status === 'DRAFT' ? '▣' : '○'}
            </Text>
          </View>
          ...
          <View style={styles.texts}>
            <View style={styles.nameRow}>
              ...
              <View style={[styles.timeChip, { backgroundColor: `${timeRemaining.color}15` }]}>
            ...
            {info.inProgress ? (
              <View style={styles.badgeRow}>
                <Animated.View style={[styles.statusBadge, { 
                  backgroundColor: info.status === 'PLACED' ? '#10b981' : '#f59e0b', 
                  transform: [{ scale }], 
                  opacity
                }]}> 
                ...
                <View style={[styles.orderIdBadge, { backgroundColor: '#f3f4f6' }]}>
  ...
  console.log('[Providers] Renderizando lista con', data?.length || 0, 'proveedores');
          ...
          <View style={styles.headerTop}>
            <View style={styles.headerLeft}>
              ...
              <View style={styles.headerStatusBadge}>
                <View style={[styles.statusDot, { backgroundColor: token ? '#10b981' : '#ef4444' }]} />
          ...
          ListEmptyComponent={
            !loading ? (
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.debugInfo}>
                  Estado: {state.isAuthenticated ? 'Autenticado' : 'No autenticado'}
                </Text>
              </View>
            ) : null
          }
            ...
            <ScrollView style={styles.modalContent}>
              ...
              {showDatePicker && (
                <DateTimePicker
                  ...
                  display="default"
                  onChange={(event, date) => {
                    console.log('[History] 📅 DatePicker onChange:', { event: event.type, date });
                    setShowDatePicker(false);
                    if (date) {
                      console.log('[History] 📅 Nueva fecha seleccionada:', date);
                ...
                <View style={styles.providerSelectContainer}>
                  {data?.map((provider) => (
                    <Pressable
                      ...
                      onPress={() => {
                        console.log('[History] 🏪 Proveedor seleccionado:', provider.id, '-', provider.name);
                        setSelectedProvider(provider.id);
                      }}
                    >
                      <View style={[
                        styles.providerRadio,
                        selectedProvider === provider.id && styles.providerRadioSelected
                      ]}>
                      ...
                      <Text style={[
                        ...
                        selectedProvider === provider.id && styles.providerOptionTextSelected
                      ]}>
                        {provider.name}
                      </Text>
                    </Pressable>
              ...
              {/* Botón de Búsqueda */}
              <Pressable
                style={[styles.searchButton, loadingHistory && styles.searchButtonDisabled]}
                onPress={() => {
              ...
              {historicalOrders.length > 0 && (
                <View>
                  <Text style={styles.resultTitle}>Órdenes Encontradas ({historicalOrders.length})</Text>
                  {historicalOrders.map((order, orderIndex) => (
                    <View key={order.id} style={styles.resultContainer}>
                      {/* Header de la orden */}
                      <View style={styles.orderHeader}>
                        <Text style={styles.historyOrderIdText}>Orden #{order.id}</Text>
                        <View style={[styles.statusChip, { 
                          backgroundColor: order.status === 'PLACED' ? '#10b981' : 
                      ...
                      <View style={styles.resultInfo}>
                        <View style={styles.resultRow}>
                          ...
                          <Text style={styles.resultValue}>{order.provider_name}</Text>
                        </View>
                        <View style={styles.resultRow}>
                          <Text style={styles.resultLabel}>Ordenado por:</Text>
                          <Text style={styles.resultValue}>{order.ordered_by_username}</Text>
                        </View>
                        <View style={styles.resultRow}>
                          <Text style={styles.resultLabel}>Fecha creación:</Text>
                      ...
                      {order.items.map((item: HistoricalOrderItem, index: number) => (
                        <View key={item.id} style={styles.itemCard}>
                          ...
                          <View style={styles.itemRow}>
                            ...
                            <Text style={styles.itemValue}>{item.quantity_units} {item.purchase_unit}</Text>
                          </View>
                          <View style={styles.itemRow}>
                            <Text style={styles.itemLabel}>Cajas:</Text>
                            <Text style={styles.itemValue}>{item.amount_boxes}</Text>
                          </View>
                          {item.notes && (
                            <View style={styles.itemRow}>
...
const styles = StyleSheet.create({
  ...
  chevron: { fontSize: 20, color: '#9ca3af', fontWeight: 'bold', marginLeft: 8 },
  header: { 
    backgroundColor: '#fff', 
  ...
  center: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#f8fafc', padding: 16 },
  loadingText: { marginTop: 12, color: '#6b7280', fontSize: 16 },
  errorText: { color: '#ef4444', marginBottom: 12, fontWeight: '600', textAlign: 'center' },
  retryBtn: { backgroundColor: '#10b981', borderRadius: 10, paddingHorizontal: 16, paddingVertical: 10 },
