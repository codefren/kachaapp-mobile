...
export default function ReceptionScreen() {
  ...
  const fetchProviders = useCallback(async () => {
    ...
    console.log('[ReceptionScreen] 🔑 Token preview:', currentToken?.substring(0, 20) + '...');
    ...
    try {
      ...
      console.log('┌─────────────────────────────────────────────────────────────┐');
      console.log('│ RESPONSE COMPLETA DEL ENDPOINT                              │');
      console.log('└─────────────────────────────────────────────────────────────┘');
      console.log('[ReceptionScreen] 📦 Response completa:', JSON.stringify(response, null, 2));
      console.log('');
      ...
      console.log('[ReceptionScreen] 🔢 Data Length:', Array.isArray(response.data) ? response.data.length : 0);
      console.log('');
      
      if (response.success && response.data) {
        const providers = response.data;
        console.log(`[ReceptionScreen] ✅ Loaded ${providers.length} providers`);
        console.log('');
        
        if (Array.isArray(providers)) {
          console.log('┌─────────────────────────────────────────────────────────────┐');
          console.log('│ DETALLE DE CADA PROVEEDOR                                   │');
          console.log('└─────────────────────────────────────────────────────────────┘');
          providers.forEach((provider, index) => {
            console.log(`\n[ReceptionScreen] 👤 Provider ${index + 1}/${providers.length}:`);
            console.log('  ├─ ID:', provider.id);
            console.log('  ├─ Name:', provider.name);
            console.log('  ├─ has_draft_reception:', provider.has_draft_reception);
            console.log('  ├─ draft_reception_order_id:', provider.draft_reception_order_id);
            console.log('  ├─ last_shipped_order_id:', provider.last_shipped_order_id);
            console.log('  ├─ has_received_orders:', JSON.stringify(provider.has_received_orders, null, 2));
            console.log('  └─ order_available_dates:', JSON.stringify(provider.order_available_dates, null, 2));
          });
          console.log('\n═══════════════════════════════════════════════════════════════');
        ...
        const providersWithShippedOrders = providers.filter(p => p.last_shipped_order_id);
        console.log(`[ReceptionScreen] 📦 Proveedores filtrados con pedidos enviados: ${providersWithShippedOrders.length}`);
            ...
            {hasReceivedOrders ? (
              <View style={styles.badgeRow}>
                ...
                <View style={[styles.orderIdBadge, { backgroundColor: '#f3f4f6' }]}>
                  <Text style={styles.orderIdText}>#{hasReceivedOrders.order_id}</Text>
                </View>
              </View>
            ) : (
